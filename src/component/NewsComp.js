...
export class NewsComp extends Component {
  ...
  fetchData=async(value)=>{
    ...
    this.setState({ articles: parsedata,// setState() will change the state to the parsedata which contains json data
    totalArticles: parsedata.length, // this will fetched total number of articles available 
    Loading:false
       })
  ...
  render() {
    return (
      ...
      <div className='container ' id='body'>
        ...
        <h2 className='  text-uppercase top-head'> TOP HEADLINES  :          {this.props.category} </h2>
        <h6 style={{color:"grey",}}  className='top-head'>Showing {this.state.totalArticles} results </h6>  {/* showing the total number of articles available for reading  */}
       ...
       {this.state.Loading&&<Loading/>}  {/* if loading state is true then it will show the loading spinner */}
        ...
        <div className="row my-4">
          ...
          return <div className="col-md-4 items-cont" key={element.url}>  {/* we have to pass an unique key so that state can differentiate in between every object of an array */}
